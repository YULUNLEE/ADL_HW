...
def main(test_dir):
    ...
    dataset=Data.TensorDataset(sentence_tensor, label_tensor)
    ...
    for epoch in range(EPOCH):
        ...
        for step, (b_x, b_y) in enumerate(loader):
            b_x = b_x.view(-1, avr_word_len, 300).to('cuda')  # reshape x to (batch, time_step, input_size)
            output= lstm(b_x)  # rnn output
            # h_n = Variable(h_n.data.to('cuda'))
            # h_c = Variable(h_c.data.to('cuda'))
            # print('1', output.shape)
            # print('2', b_y.view(-1).shape)
            loss = loss_function(output, b_y.view(-1).to('cuda'))  # cross entropy loss
            optimizer.zero_grad()  # clear gradients for this training step
            loss.backward()  # backpropagation, compute gradients
            optimizer.step()  # apply gradients
            if step % 100 == 0:
                count=0
                output = lstm(b_x)  # (samples, time_step, input_size)
                # print('h_n', h_n)
                # print('h_c', h_c)
                ...
                print(f'Epoch:{epoch} | step:{step} | train loss:{loss.data.cpu().numpy()} | train accuracy: {accuracy}%')

    # save model
    torch.save(lstm, 'intent_model.pkl')
...
if __name__ == "__main__":
    ...
    # main(args)
    output_ans, output_id = main(args.test_dir)
    # print('id:', output_id)
