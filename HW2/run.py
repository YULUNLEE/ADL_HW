...
def covert_to_squad_json(context_file, input_file, output_squad_file):
    sample_dict = load_json("sample_train.json")
    dict = load_json(input_file)
    for i in tqdm(range(len(dict) - 3)):
        sample_dict['data'].append({'paragraphs': [{'context': 'abcde', 'qas': [
            {'question': 'qwe', 'id': '181543_1', 'answers': [{'text': 'ab', 'answer_start': 0}]}]}]})

    count = 0

    for num in range(len(dict)):

        # print(dict[num]['paragraphs'])
        # print(dict[num]['question'])
        sample_dict['data'][count]['paragraphs'][0]['context'] = dict[num]['paragraphs']
        sample_dict['data'][count]['paragraphs'][0]['qas'][0]['question'] = dict[num]['question']
        # sample_dict['data'][count]['paragraphs'][0]['qas'][0]['answers'][0]['text'] = dict[num]['answers'][0]['text']
        sample_dict['data'][count]['paragraphs'][0]['qas'][0]['answers'][0]['text'] = None
        # sample_dict['data'][count]['paragraphs'][0]['qas'][0]['answers'][0]['answer_start'] = dict[num]['answers'][0]['start']
        sample_dict['data'][count]['paragraphs'][0]['qas'][0]['answers'][0]['answer_start'] = None
        sample_dict['data'][count]['paragraphs'][0]['qas'][0]['id'] = dict[num]['id']

        count += 1

    print('len=', count)

    save_json(sample_dict, './data/my_sample_dev.json')

    # 讀入 context
    start_lengh = 0
    dict = load_json(input_file)
    context_dict = load_json(context_file)
    train_dict = load_json('./data/my_sample_dev.json')

    for i in range(len(train_dict['data'])):
        # make start
        '''
        for step, j in enumerate(train_dict['data'][i]['paragraphs'][0]['context']):

            if j != dict[i]['relevant']:
                start_lengh += len(context_dict[j])
                # print(len(context_dict[j]))
                # print('A', start_lengh)
            else:
                train_dict['data'][i]['paragraphs'][0]['qas'][0]['answers'][0]['answer_start'] = start_lengh + int(
                    train_dict['data'][i]['paragraphs'][0]['qas'][0]['answers'][0]['answer_start'])
                start_lengh = 0
                break
        '''
        for step, j in enumerate(train_dict['data'][i]['paragraphs'][0]['context']):
            # context append
            if step == 0:
                train_dict['data'][i]['paragraphs'][0]['context'] = context_dict[j]
            else:
                train_dict['data'][i]['paragraphs'][0]['context'] = train_dict['data'][i]['paragraphs'][0][
                                                                        'context'] + str(context_dict[j])

        # print(train_dict['data'][i]['paragraphs'][0]['context'])
        # print(len(train_dict['data'][i]['paragraphs'][0]['context']))
        # print(len(context_dict[5234])+len(context_dict[6952])+len(context_dict[8264])+len(context_dict[836])+len(context_dict[92])+len(context_dict[2018]))
        # print(train_dict['data'][i]['paragraphs'][0]['qas'][0]['answers'][0]['answer_start'])

    save_json(train_dict, output_squad_file)
        ...
        for paragraph in entry["paragraphs"]:
            ...
            for qa in paragraph["qas"]:
                ...
                if is_training:
                    ...
                    # print('0', doc_tokens)
                    # print('1', cleaned_answer_text)
                    # print('2', ori_start_position)
...
def write_to_file(all_predictions, output_prediction_file):
...
def run_evaluate(args, model, eval_features, device, eval_examples, tokenizer, best_dev_score=None):
    ...
    # output_prediction_file = os.path.join(args.output_dir, "predictions_private.json")
    # output_nbest_file = os.path.join(args.output_dir, "nbest_predictions_private.json")
    output_prediction_file = args.output_dir
    # output_nbest_file = "output/nbest_predictions_private.json"
    predict, nbest = write_predictions(eval_examples, eval_features, all_results,
                                       args.n_best_size, args.max_answer_length,
                                       args.do_lower_case, output_prediction_file,
                                       args.verbose_logging)

    write_to_file(predict, output_prediction_file)
    #無法評估因為沒有答案
    # with open(args.predict_file, encoding='utf-8') as dataset_file:
    #     dataset_json = json.load(dataset_file)
    #     dataset = dataset_json['data']
    # dev_score_EM = evaluate(dataset, predict, tokenizer)['exact_match']
    # dev_score_f1 = evaluate(dataset, predict, tokenizer)['f1']
    # logger.info('Dev EM score : {}'.format(dev_score_EM))
    # logger.info('Dev f1 score : {}'.format(dev_score_f1))


    # kwargs = parse_args()
    # global times
    # if times==0 and  best_dev_score is not None:
    #     kwargs = parse_args()
    ...
    #     dev_score_EM, dev_score_f1 = eval_TA.pred_score(**kwargs)
    #     best_dev_score = dev_score_EM
    #     times=1
    #     logger.info('Saving model with dev EM score: {}'.format(dev_score_EM))
    #
    #
    #
    # dev_score_EM, dev_score_f1 = eval_TA.pred_score(**kwargs)
    ...
    # if best_dev_score is not None and dev_score_EM > best_dev_score and times != 0:
    #     logger.info('Saving model with dev EM score: {}'.format(dev_score_EM))
    #     best_dev_score = dev_score_EM
    #     write_to_file(predict, nbest, output_prediction_file, output_nbest_file)
    #     torch.save(model.state_dict(), args.finetuned_checkpoint)
...
def main():
    ...
    parser.add_argument("--output_dir", default='output/dev_predictions.json', type=str,
    ...
    ## Other parameters
    parser.add_argument("--context_file", default='./data/context.json', type=str,
                        help="TA train json to squad 1.1 version json ")
    parser.add_argument("--input_file", default='data/private.json', type=str,
                        help="TA train json to squad 1.1 version json ")
    parser.add_argument("--output_squad_file", default='data/squad_dev.json', type=str,
    ...
    parser.add_argument("--max_seq_length", default=200, type=int,  #384
                        help="The maximum total input sequence length after WordPiece tokenization. Sequences "
                             "longer than this will be truncated, and sequences shorter than this will be padded.")
    parser.add_argument("--doc_stride", default=50, type=int,
    ...
    parser.add_argument("--warmup_proportion", default=0.1, type=float,
                        help="Proportion of training to perform linear learning rate warmup for. E.g., 0.1 = 10% "
                             "of training.")
    parser.add_argument("--save_checkpoints_steps", default=4000, type=int,
    ...
    #load TA data and create squad data
    covert_to_squad_json(args.context_file, args.input_file, args.output_squad_file)
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if (step + 1) % args.save_checkpoints_steps == 0:
                    best_dev_score = run_evaluate(args, model, eval_features, device, eval_examples, tokenizer, best_dev_score)
